...
namespace VoreMod
{
    ...
    public class VoreConfig : ModConfig
    {
		...
		[DefaultValue(true), Label("Disposal gore")]
		public bool EffectsDisposalGore;
    ...
    public class VoreTagList
    {
        ...
        public bool Match(VoreEntity entity)
        {
            ...
            foreach (VoreTagListItem item in allow) matches |= item.Match(entity);
            foreach (VoreTagListItem item in disallow) matches &= !item.Match(entity);
            return matches;
        }
    }
