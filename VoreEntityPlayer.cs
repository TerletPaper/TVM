...
namespace VoreMod
{
    public class VoreEntityPlayer : VoreEntity
    {
		...
		public override void OnDispose(VoreEntity prey){
            if(player.armor[0].type == ItemID.BossMaskBetsy) {
                int p = Projectile.NewProjectile(player.MountedCenter+new Vector2(player.direction*2, -24), new Vector2(player.direction*6, -0.5f), ProjectileID.Flames, prey.GetLifeMax(), 3, player.whoAmI);
                Main.projectile[p].friendly = true;
                Main.projectile[p].hostile = false;
                Main.projectile[p].trap = false;
                Main.projectile[p].timeLeft/=8;
                Main.projectile[p].ai[0]+=6;
            }
		}
		public override void OnBurp(){
            if(player.armor[0].type == ItemID.BossMaskBetsy) {
                int p = Projectile.NewProjectile(player.MountedCenter+new Vector2(player.direction*2, -24), new Vector2(player.direction*6, -0.5f), ProjectileID.Flames, 30, 3, player.whoAmI);
                Main.projectile[p].friendly = true;
                Main.projectile[p].hostile = false;
                Main.projectile[p].trap = false;
                Main.projectile[p].timeLeft = 8;
        ...
        public override float GetSizeFactor() => player.width * player.height / 800f;

        public override void Damage(VoreEntity damager, int damage, float knockback)
        {
            PlayerDeathReason reason = PlayerDeathReason.ByCustomReason(string.Format(GetRandomDeathMessage(), this, damager));
        ...
        public override int GetEscapeBonus(VoreEntity prey) {
			IEnumerable<Item> arg_2B_0 = this.GetPlayer().inventory;
			IEnumerable<AmuletBase> arg_54_0 = arg_2B_0.Select((i)=>i.modItem).OfType<AmuletBase>();
            return arg_54_0.Max((i) => i.EscapeBonus);
        }
        ...
        internal string GetRandomDeathMessage()
        {
